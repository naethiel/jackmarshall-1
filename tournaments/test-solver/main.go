...
func main() {
	...
	for i := 0; i < nbRound; i++ {
		...
		pairing, ite := sol.Solve(players)
		fmt.Printf("Pairing done in %d iterations with a fitness score of %.0f\n", ite, pairing.Fitness)
		pairs := PairsFromPlayers(pairing.Genes.(Players))

		assignements, j := sol.Solve(Assignements{Pairs: pairs, Tables: t.Tables})
		fmt.Printf("Assignements done in %d iterations with a fitness score of %.0f\n", j, assignements.Fitness)
